...
export const ResetPassword = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (email && otp && !password) {
      const user = await prisma.otpVerification.findUnique({
        where: { email },
      });
      if (!user) {
        throw new APIError({ status: 401, message: "Invalid email" });
      ...
      res.status(200).json(
        new ApiResponse({
          data: "",
          statusCode: 200,
          message: " create the new password now",
        })
      );
    }
...
export const validate = asyncHandler(async (req: Request, res: Response) => {
  ...
  res
    .status(200)
    .json(new ApiResponse({ statusCode: 200, data: { role: decoded.role } }));
});
